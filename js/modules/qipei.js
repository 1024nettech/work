...
function guigetu_upload() {
    // Create a single upload button for guigetu (规格图)
    $("#guigetu").click(function () {
        ...
        $fileInput.off('change').on('change', function (event) {
            let files = event.target.files;
            let fileIndex = 0; // Track which file to upload
            let uploadedUrls = []; // Store the URLs for the uploaded images
    ...
    function uploadImage(imageBlob, filename, index, resolve, reject) {
        ...
        formData.append('chunk', 0);
        formData.append('chunks', 1);
        let url = location.origin + "/Ajax/plupload.php?username=qipeiyigouwang&type=10";  // Update URL with type=10
        ...
        fetch(url, {
            ...
            .then(responseText => {
                ...
                // Add the resized URL to the uploadedUrls array
                uploadedUrls.push(resizedUrl);

                // Immediately update the DOM after uploading
                updateDOM(uploadedUrls, index, files.length); // Pass index and total image count
                resolve(); // Resolve the promise to proceed with the next image
            })
            .catch(error => {
                console.log('Upload failed:', error);
    ...
    function updateDOM(uploadedUrls, currentIndex, totalFiles) {
        console.log(`Updating DOM for image index ${currentIndex}, with URLs: ${uploadedUrls}`);

        // Calculate the total number of rows in the table
        let totalRows = $('#combination_table tr').length;

        // Loop through each row and apply the uploaded URLs
        $('#combination_table tr[data-sn]').each(function (index) {
            let tableRow = $(this);
            let rowIndex = index + 1; // Adjust for 0-based index

            // We repeat the image URLs as needed (e.g., if we have 3 images and 6 rows)
            let imageUrl = uploadedUrls[(rowIndex - 1) % uploadedUrls.length]; // Repeat images if fewer images than rows

            // Update the image src with the resized URL
            tableRow.find('.img-td img').attr('src', imageUrl);

            // Set the input value to the image URL without query parameters
            tableRow.find('.img-td>input').val(imageUrl.split('?')[0]);

            // Remove any styles from the .del-tag (ensure it's visible)
            tableRow.find('.img-td .del-tag').css('style', ''); // Remove any style settings (like visibility:hidden)
        });
    }
}
export { open_close_shop_products, showKeyword, fetchChIdsAndTitles, checkProduct, zhutu_upload, guigetu_upload }
// End-384-2025.11.28.153302
