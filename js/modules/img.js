...
function rename() {
    ...
    if (url.includes("https://detail.1688.com/offer/")) {
        ...
        let html = `${$(".sku-title-name").text().trim()}|@@|${$("meta[name=keywords]").attr("content").split(",")[0]}|@@|${$(".p-price jdPrice span:last").text().trim()}|@@|件|@@|${$("#buy-num").val()}|@@|10000|@@|${$("#J-detail-content").html()}`;
