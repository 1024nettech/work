...
function rename() {
    ...
    if (url.includes("https://detail.1688.com/offer/")) {
        ...
        let html = `${$("span[class^=mainTitle--]").text().trim()}|@@|${$("span[class^=mainTitle--]").text().trim()}|@@|${$("span[class^=symbol--]").first().next("span").text().trim()}|@@|件|@@|${$("input[class^=countValue--]").val()}|@@|10000|@@|${$("#content").html()}`;
